...
export default async function PublicQuotePage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ accepted?: string; rejected?: string; error?: string }> }) {
  ...
  if (quote.status === 'SENT') {
    quote = await prisma.quote.update({ where: { id: quote.id }, data: { status: 'VIEWED' }, include: { items: { orderBy: { position: 'asc' } } } })
  }
  ...
  const canAnswer = ['SENT', 'VIEWED'].includes(quote.status)
  ...
  return <main className="py-10 md:py-16 print:bg-white print:py-0">
    <div className="container print:w-full print:max-w-none">
      <div className="mx-auto max-w-5xl">
        ...
        {query.accepted && <div className="mb-6 rounded-3xl bg-emerald-100 p-5 font-bold text-emerald-950 print:hidden">Merci. Le devis a bien été accepté.</div>}
        {query.rejected && <div className="mb-6 rounded-3xl bg-black/[0.05] p-5 font-bold print:hidden">Votre refus a bien été enregistré.</div>}
