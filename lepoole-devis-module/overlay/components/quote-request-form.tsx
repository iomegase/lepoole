...
export function QuoteRequestForm({ action }: Props) {
  ...
  async function uploadPhotos() {
    ...
    for (const file of files) {
      ...
      if (!response.ok || !data.path) throw new Error(data.error || 'Envoi de photo impossible.')
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      for (const path of paths) data.append('photoPath', path)
      startTransition(async () => action(data))
    } catch (cause) {
  ...
  return (
    <form ref={formRef} onSubmit={submit} className="rounded-[32px] border border-black/[0.08] bg-white p-5 shadow-[0_24px_70px_rgba(0,0,0,.06)] md:p-8">
      ...
      <div className="mb-8 grid grid-cols-3 gap-2">
        {['Coordonnées', 'Projet', 'Photos'].map((name, index) => {
          ...
          return (
            <button key={name} type="button" onClick={() => done && setStep(number)} className="text-left">
      ...
      <div data-step="2" className={step === 2 ? 'grid gap-5' : 'hidden'}>
        <div>
          ...
          <p className="mt-1 text-sm text-black/50">Ces informations aident Stan à étudier la demande et le contexte de TVA.</p>
        </div>
      ...
      <div data-step="3" className={step === 3 ? 'grid gap-5' : 'hidden'}>
        ...
        <label className="block cursor-pointer rounded-3xl border border-dashed border-black/20 bg-[#f6f3ec] p-8 text-center transition hover:border-black/40">
      ...
      <div className="mt-8 flex items-center justify-between gap-3 border-t border-black/10 pt-6">
        ...
        {step < 3 ? (
          <button type="button" onClick={next} className="rounded-full bg-[#171717] px-6 py-3 text-sm font-black text-white">Continuer</button>
        ) : (
